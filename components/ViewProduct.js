...
export default function ViewProducts({ products, productHeaders, categories }) {
    ...
    const onDetailClick = (id, name, sub) => {
        ...
        }, `/product/${id}`, { shallow: true, as: router.asPath }, { scroll: false })
    }
    const onEditClick = (id, name, sub) => {
        ...
        }, `/editproduct/${id}`, { shallow: true, as: router.asPath }, { scroll: false })
    }
    const onDeleteClick = async (id) => {
        try {
            message.loading({ content: 'Deleting...', duration: 2 });


            // Introduce a delay of 2 seconds before the delete action
            await new Promise((resolve) => setTimeout(resolve, 2000));

            const api = await fetch('https://kabstore-7p9q.onrender.com/product/' + id, {
                method: 'DELETE',
            });
    ...
    useEffect(() => {
        ...
        return setDisplayProducts(data?.slice(firstPageIndex, lastPageIndex));
                                                ...
                                                <td>
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <MenuItem onClick={() => {
                                                                handleClose()
                                                                onEditClick(product._id, "Catalog", "Products")
                                                            ...
                                                            <MenuItem onClick={() => {
                                                                handleClose()
                                                                onDetailClick(product._id, "Catalog", "Products")
                                                            }} disableRipple>
                                                                <FileCopyIcon />
                                                                Details
                                                            </MenuItem>
